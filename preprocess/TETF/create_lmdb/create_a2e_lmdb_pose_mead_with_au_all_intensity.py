...
class TalkingA2VqvaeSeqKeylaBase(Dataset):
    ...
    def _load_mead(self,
        ...
        # for each video clip
        for latent_file in tqdm(latent_files[:int(0.33*len(latent_files))]):
            # get file name
            file_name = os.path.basename(latent_file).split('.')[0].strip('motion_')
            # check if audio feature exists
...
if __name__ == "__main__":
    ...
    save_dir = '/xxxx/TETF/datasets/lmdbs_uni_pose_au/mead_lmdb_all_au_pose_train_subset'
    dataset_name = 'mead_lmdb_all_au_pose_train_subset'
