...
'''
Author             : Torchaudio
Date               : 2023-03-20 06:33
Last Modified By   : Tianyu He (xxxx@microsoft.com)
Last Modified Date : 2023-03-31 10:43
...
from talking.modules.vqvae.conformerfairseq import RelPositionMultiHeadedAttention, RelPositionalEncoding, PositionalEmbedding
...
class ConformerLayerconcateLayer(torch.nn.Module):
    ...
    def forward(self, input, key_padding_mask, cond, position_emb=None):
        r"""
        NOTE: support cond=None, which will degrade to the original conformer.
        Args:
            input (torch.Tensor): input, with shape `(T, B, D)`.
...
class CondConformer(torch.nn.Module):
    ...
    *Conformer: Convolution-augmented Transformer for Speech Recognition*
    ...
    def __init__(
        ...
        self.use_rel_attn = use_rel_attn
        if use_rel_attn:
            self.embed_positions = RelPositionalEncoding(max_source_positions, input_dim)
    ...
    def forward(self, input, masks, cond):
        r"""
        ...
        for layer in self.conformer_layers:
            if cond is not None:
                x = layer(x, encoder_padding_mask, cond.transpose(0, 1), position_emb)
            else:
                # layerconcat support cond=None
                x = layer(x, encoder_padding_mask, None, position_emb)
        return x.transpose(0, 1)