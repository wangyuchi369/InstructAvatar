...
def main():
    ...
    val_mapping_dict = json.load(open(os.path.join(opt.test_set_dir, 'mapping_dict.json')))
    
    
    for person in os.listdir(os.path.join(opt.test_set_dir, 'frames_256')):

        if person not in val_mapping_dict:
            continue
       
        video_name = val_mapping_dict[person]
        mead_dir = '/mnt/blob/xxxx/TETF/datasets/MEAD_extend/'
        mead_person, mead_video = video_name.split('_')[0], '_'.join(video_name.split('_')[1:])

        opt.appear_dir = os.path.join(opt.test_set_dir, 'frames_256', person)
        if opt.use_mead_audio:
            opt.audio_path = os.path.join(mead_dir, 'MEAD_all_randcrop', mead_person, "audio_16k", f'{mead_video}.wav')
            opt.audio_feat = os.path.join(mead_dir, 'audio_wave2vec_all', f'{mead_person}_{mead_video}.npy')
        else:
            opt.audio_path = os.path.join(opt.test_set_dir, 'audio_16k', f'{person}.wav')
            opt.audio_feat = os.path.join(opt.test_set_dir, 'audio_16k_single_normed_wav2vec', f'{person}.npy')
        try:
            au = list(json.load(open(os.path.join(mead_dir, 'au_detect', mead_person, mead_video, 'intersection.json'))).values())[0]
        except:
            print(f'{video_name} has no au')
            continue
        ...
        opt.video_name = os.path.join(opt.video_save_dir, 'single_video', f'{person}_{video_name}.mp4')
        full_video_path = os.path.join(opt.video_save_dir, 'full_video', f'{person}_{video_name}.mp4')
