...
class TwoCondResidualBlock(nn.Module):
    ...
    def forward(self, x, cond_audio, cond_keyla, diffusion_step):
        ...
        if self.is_audio_add:
            add_cond = self.add_projection(cond_audio)
            concat_cond = self.concat_projection(cond_keyla)
        else:
            add_cond = self.add_projection(cond_keyla)
            concat_cond = self.concat_projection(cond_audio)
...
class AdaNormResidualBlock(nn.Module):
    ...
    def forward(self, x, cond_audio, cond_keyla, diffusion_step):
        ...
        y = self.norm1(self.dilated_conv(y)) + self.add_cond_projection(cond_keyla)
