...
from get_landmark import regress_landmarks
import warnings
...
def main():
    ...
    parser.add_argument(
        ...
        #default="/mnt/blob/xxxx/diffusion/stable-diffusion/latents/seperate_ldmk_ccd_set2_ep234.ckpt",
        # default="/data/datasets/AVENA_CCD_HDTF/latents-700m-avena-ep23/seperate_ldmk_scale_avena_dense_vae_set2_ep23.ckpt",
        default="/mnt/blob/xxxx/diffusion/stable-diffusion/latents/seperate_ldmk_scale_avena_dense_vae_set2_randft_ep12.ckpt",
    ...
    parser.add_argument(
        ...
        default="/mnt/blob/xxxx/dataset/text2motion/MEAD_clean/M003/audio_16k/happy_011_0.wav",
        help="audio path",
    )
    parser.add_argument(
        "--video_name",
    ...
    parser.add_argument(
        "--test_set_dir",
        type=str,
        default="~/TETF/datasets/test_set",
        help="the test set dir"
    )
    parser.add_argument(
        "--only_emotion_instruction",
        type=str2bool,
        default=False,
        help="whether to only use emotion instruction"
    )
    parser.add_argument(
        "--only_au_instruction",
        type=str2bool,
        default=False,
        help="whether to only use au instruction"
    )
    parser.add_argument(
        "--video_save_dir",
        type=str,
        default="~/TETF/datasets/test_set",
        help="the video save dir"
    )
    parser.add_argument(
        "--use_subset",
        type=str2bool,
        default=True,
        help="use subset of test set or not"
    )
    parser.add_argument(
        "--use_eos",
        type=str2bool,
    ...
    for person in os.listdir(os.path.join(opt.test_set_dir, 'frames')):
        # assert person in ['M003', "M019", 'W038', "W018", "M007"], f'{person} not in test set'
        if person not in ['M003', 'M019', 'W038', 'W018', 'M007']:
            continue
        # for video_name in tqdm(os.listdir(os.path.join(opt.test_set_dir, 'frames', person))):
        if opt.use_subset:
            to_evaluate_json = json.load(open(os.path.join(opt.test_set_dir, 'test_set.json')))
            evaluate_videos = to_evaluate_json[person]
        else:
            evaluate_videos = os.listdir(os.path.join(opt.test_set_dir, 'frames', person))
        for video_name in tqdm(evaluate_videos):
            video_name = video_name.replace('.mp4', '')
            print(f'Processing {person}/{video_name}')
            #opt.appear_dir = os.path.join(opt.test_set_dir, 'frames', person, 'neutral_001_0')
            opt.appear_dir = os.path.join(opt.test_set_dir, 'frames', person, video_name)
            opt.audio_path = os.path.join(opt.test_set_dir, 'MEAD_all_randcrop', person, "audio_16k", f'{video_name}.wav')
            opt.audio_feat = os.path.join(opt.test_set_dir, 'audio_wave2vec_all/', f'{person}_{video_name}.npy')
            try:
                au = list(json.load(open(os.path.join(opt.test_set_dir, 'au_detect', person, video_name, 'intersection.json'))).values())[0]
            except:
                print(f'{person}/{video_name} has no au')
                continue
            emotion = video_name.split('_')[0]
            if not au:
                print(f'{person}/{video_name} has no au')
                continue
            action_units_sentence = '; '.join([" ".join(i.split('_')) for i in au])
            if opt.only_emotion_instruction:
                opt.instruction = f'please talk with {emotion} emotion'
            elif opt.only_au_instruction:
                opt.instruction = f'please talk with following action units: {action_units_sentence} '
            else:
                opt.instruction = f'please talk with {emotion} emotion and following action units: {action_units_sentence} '
            
            opt.video_name = os.path.join(opt.video_save_dir, f'{person}/{video_name}.mp4')
            full_video_path = os.path.join(opt.video_save_dir, 'full_video', person, f'{video_name}.mp4')
            ...
            # read appear img
            appear_img_path = sorted(glob(f'{opt.appear_dir}/*'))[0]
                ...
                with torch.no_grad():
                    ...
                    if opt.a2e_predict_ldmk:
                        ...
                        if opt.a2e_use_both_keymo_keyappear:
                            ...
                            # print('here!')
                            # motion_latent = torch.from_numpy(np.load('/mnt/blob/xxxx/TETF/datasets/text2motion/text2motion_latents_700M_ep12/motion_CC_part_10_1_1142_01_8_0.npy')).reshape(1,-1,768).to(device)
                            # # motion_latent = torch.from_numpy(np.load('/mnt/blob/xxxx/TETF/datasets/MEAD/MEAD_latents_700M_ep12/motion_M003_neutral_006_0.npy')).reshape(1,-1,768).to(device)
            ...
            final_video = clips_array([[appear_video, recon_video]])
            if opt.a2e_predict_ldmk:
                ...
                final_video = clips_array([[appear_video, ldmk_video, recon_video]])
            tmp = copy.deepcopy(final_video)
            tmp.audio = AudioFileClip(opt.audio_path, fps=16000).set_duration(tmp_audio_duration)
            tmp.write_videofile(full_video_path)
