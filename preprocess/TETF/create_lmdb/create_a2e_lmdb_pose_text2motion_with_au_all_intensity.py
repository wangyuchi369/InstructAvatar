...
class TalkingA2VqvaeSeqKeylaBase(Dataset):
    def __init__(self,
        ...
        self._load_t2m(t2m_latent_dir_name, t2m_audio_feat_dir_name)
    ...
    def _load_hdtf(self,
        ...
        latent_files = sorted(glob.glob(os.path.join(self.data_root, hdtf_latent_dir_name, 'motion*.npy')))
        # for each video clip
        for latent_file in tqdm(latent_files[int(0.47*len(latent_files)):int(0.5*len(latent_files))]):
            # get file name
    ...
    def _load_t2m(self, t2m_latent_dir_name, t2m_audio_feat_dir_name):
        ...
        for latent_file in tqdm(latent_files[:int(0.4*len(latent_files))]):
            # get file name
            file_name = os.path.basename(latent_file).split('.')[0].strip('motion_')
            ...
            # if file_name in ccd_audio_feat_files:
                ...
                audio_feat = np.load(os.path.join(self.data_root, t2m_audio_feat_dir_name, file_name + '.npy')).T  # (2frames, 512)
                ...
                appear_latent = None
                audio_feat = None
            if self._check_t2m_latent(motion_latent,  appear_latent, ldmk, audio_feat,  file_name) and self._check_file_name(file_name) and self._check_instruction(instruction):
                single_video_data = dict()
                single_video_data['filename'] = file_name
                ...
                single_video_data['audio_feat_path'] = os.path.join(self.data_root, t2m_audio_feat_dir_name, file_name + '.npy')
                single_video_data['instruction'] = instruction
...
if __name__ == "__main__":
    ...
    t2m_audio_feat_dir_name = 'text2motion/audio_wave2vec_all'
    
    save_dir = '/xxxx/TETF/datasets/lmdbs_uni_pose_au/text2motion_lmdb_all_au_pose_train_subset'
    dataset_name = 'text2motion_lmdb_all_au_pose_train_subset'
    ...
    dataset = TalkingA2VqvaeSeqKeylaBase(data_root=data_root,
                                        ...
                                        #  ccdv2_audio_feat_dir_name=ccdv2_audio_feat_dir_name,
                                         ...
                                         t2m_audio_feat_dir_name=t2m_audio_feat_dir_name,
                                         min_num_frames=25)
