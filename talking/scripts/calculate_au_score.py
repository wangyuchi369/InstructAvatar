...
for person in os.listdir(opt.result_dir):
    if person == 'generated_au' or person == 'lip_sync_saved' or person == 'final_results.txt' or person == 'full_video' or person == 'single_video':
        continue
    for each_video in os.listdir(os.path.join(opt.result_dir, person)):
        video_name = each_video[:-4]
         # assert video.endswith(".mp4")
        
        current_path = os.path.join(opt.result_dir, person, each_video)

        # 打开视频文件
        video = cv2.VideoCapture(current_path)
        
        if not os.path.exists(os.path.join(opt.result_dir, 'generated_au', person, each_video[:-4], 'intersection.json')):
            ...
            saved_dir = os.path.join(opt.result_dir, 'generated_au', person, current_path.split('/')[-1].replace('.mp4', '/'))
            ...
            intersection_au = json.load(open(os.path.join(opt.result_dir, 'generated_au', person, each_video[:-4], 'intersection.json'), 'r'))['/'.join([person, each_video[:-4], ''])]
        # calculate score for each video
        gt_au_file = os.path.join('/mnt/blob/xxxx/TETF/datasets/MEAD_extend/au_detect', person, video_name, 'intersection.json')
        try:
